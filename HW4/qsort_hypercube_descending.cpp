...
// MPI-based implementation (global order: non-increasing by rank; local lists descending)
//
#include <cmath>
#include <cstdlib>
#include <cstdio>
...
// Merge two descending-sorted lists and return the merged list
// Input:
...
// Search for smallest index in a descending-sorted list where list[idx] <= pivot
// Uses binary search since list is sorted in non-increasing order.
// Input:
//   list, list_size	- list and its size
//   pivot		- value to search for
// Output:
//   idx	- list[0..idx-1] > pivot, list[idx..list_size-1] <= pivot
//
...
// Check if list is sorted (global non-increasing by rank; local non-increasing).
// Each process verifies that its local list is sorted in descending order.
// The process also checks that its largest element is <= the smallest value on
// process (my_id-1), so all values on lower ranks are >= all values on this rank.
...
void HyperCube_Class::check_list() {
    ...
    // Local descending order; boundary: max on this proc <= min on previous proc
    local_error = 0;
    if (list_size > 0) {
...
void HyperCube_Class::HyperCube_QuickSort() {
    ...
    int idx;			// index where local list is split (descending: high | low)
    int list_size_high;		// Number of elements > pivot (kept by lower rank)
    int list_size_low;		// Number of elements <= pivot (kept by higher rank)
	...
	// Descending local list: split into > pivot (left) and <= pivot (right)
	// Lower MPI rank keeps the > pivot part; higher rank keeps <= pivot.
	idx = split_list_index(list, list_size, pivot);

	list_size_high = idx;
	list_size_low = list_size - idx;

	// Communicate with neighbor along dimension k
	nbr_k = neighbor_along_dim_k(k); 

	if (nbr_k > my_id) {
	    // Lower rank: send low (<= pivot) count, receive neighbor's high count

	    // ***** Add MPI call here *****
		MPI_Send(&list_size_low, 1, MPI_INT, nbr_k, tag, MPI_COMM_WORLD);
	    ...
	    // Send list[idx ... list_size-1] (<= pivot) to higher-rank neighbor
		MPI_Send(&list[idx], list_size_low, MPI_INT, nbr_k, tag, MPI_COMM_WORLD);
	    // ***** Add MPI call here *****


	    // Receive neighbor's > pivot sublist

	    // ***** Add MPI call here *****
		MPI_Recv(&nbr_list[0], nbr_list_size, MPI_INT, nbr_k, tag, MPI_COMM_WORLD, &status);

	    // Merge local > pivot with neighbor's > pivot (both descending)
	    ...
	    delete [] nbr_list; 
	    list = new_list; 
	    list_size = list_size_high+nbr_list_size;

	} else {
	    // Higher rank: receive low count, send high count
	    ...
	    // ***** Add MPI call here *****
		MPI_Send(&list_size_high, 1, MPI_INT, nbr_k, tag, MPI_COMM_WORLD);
	    ...
	    // Send list[0 ... idx-1] (> pivot) to lower-rank neighbor

	    // ***** Add MPI call here *****
		MPI_Send(&list[0], list_size_high, MPI_INT, nbr_k, tag, MPI_COMM_WORLD);

	    // Merge local <= pivot with neighbor's <= pivot
	    new_list = merged_list(&list[idx], list_size_low, nbr_list, nbr_list_size); 

	    // Replace local list with new_list, update size
	    delete [] list; 
	    delete [] nbr_list; 
	    list = new_list; 
	    list_size = list_size_low+nbr_list_size;
	}
